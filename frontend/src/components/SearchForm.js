...
const SearchForm = ({ fantacitorio }) => {
    ...
    const { noMatch, 
            textTweets,
            isLoading } = useSelector(state => fantacitorio ? state.fantacitorio : state.tweets);
    ...
    <>
        <div className="flex flex-col w-full p-5 items-center dark:bg-gray-900">
            <form className="flex w-full flex-col justify-center items-center gap-4" onSubmit={handleSubmit(onSubmit)}>
                ...
                <SearchFilters type={type} setValue={setValue} register={register} errors={errors} noIntervalSearch={noIntervalSearch} fantacitorio={fantacitorio} />
