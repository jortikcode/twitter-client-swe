...
export function getDateInterval(data, dataToAction, now) {
  ...
  shiftedEndDate = new Date(
    shiftedEndDate + ONE_DAY_MILLISECONDS - ONE_HOUR_MILLISECONDS
  );
  // Bisogna evitare che la data shiftata vada oltre il giorno odierno
  shiftedEndDate =
    shiftedEndDate.getTime() > now ? new Date(now) : shiftedEndDate;
...
export function secondsGranularity(isoDate) {
  const isoSecondsGranularity = `${isoDate.substring(
    0,
    isoDate.length - 7
  )}00.000Z`;
...
export const isIntervalSetted = (
  ...
  return (
    (data.startDate !== oneWeekAgoIso ||
      data.endDate !== todayIso ||
      data.username) &&
    filtersEnabled &&
    !noIntervalSearch
  );
