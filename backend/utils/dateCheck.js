...
/* ritorna true se la stringa passata in output è nel fromato ISO, false altrimenti */
  ...
  try {
    ...
    if (start_time) {
      if (!isIsoDate(start_time)) {
        throw new Error(
          "Parametro start_time non valido, non è in formato iso 8601"
        );
      }
    ...
    if (end_time) {
      if (!isIsoDate(end_time)) {
        throw new Error(
          "Parametro end_time non valido, non è in formato iso 8601"
        );
      }
    ...
    if (start_time && end_time) {
      if (Date.parse(start_time) > Date.parse(end_time)) {
        throw new Error(
          "Date non valide, la data di inizio è dopo della data di fine"
        );
      }
    }
    return true;
